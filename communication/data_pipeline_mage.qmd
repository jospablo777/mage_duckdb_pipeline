...
encoded_months = encoder.fit_transform(X_train[['month']])

# Replace the original 'month' column with encoded columns
month_columns = encoder.get_feature_names_out(['month'])
X_train_encoded = pd.DataFrame(encoded_months, columns=month_columns, index=X_train.index)
X_test_encoded = pd.DataFrame(encoder.transform(X_test[['month']]), columns=month_columns, index=X_test.index)
