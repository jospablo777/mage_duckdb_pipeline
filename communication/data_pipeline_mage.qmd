...
rum_sales = conn_py.sql(query_rum_sales).df()
...
rum_sales['year_month'] = pd.to_datetime(rum_sales['year_month'])
...
rum_sales = (
    ...
    .sort_index() # <1>
    .asfreq('MS') # <2>
    .fillna({'bottles_sold': 0})
    .reset_index()
)

forecast_start = pd.Timestamp('2025-01-01') # <3>

y = (
    rum_sales
    .set_index('year_month')['bottles_sold']
    .astype(float)
    .asfreq('MS')
    .fillna(0)
)

y_train = y[y.index < forecast_start] # <4>
y_test = y[y.index >= forecast_start] # <4>
...
train_df = y_train.rename('bottles_sold').reset_index()
test_df = y_test.rename('bottles_sold').reset_index()

train_df.tail(3), test_df.head(3)
```
1. Ensure the data is in chronological order.
2. Aggregate the data to a regular monthly frequency.
3. Define the forecast period, which in this case starts in January 2025 and continues through the 2026 months available in the database.
...
def calendar_exog(dates, origin):
...
origin = y.index.min()
exog = calendar_exog(y.index, origin=origin)
...
forecast_log = forecaster.predict_interval(
    steps=len(y_test),
    exog=exog_test,
...
forecast_log.index = y_test.index

forecast_results = pd.DataFrame(
    {
        'year_month': y_test.index,
        'bottles_sold': y_test.to_numpy(),
        'forecast': np.expm1(forecast_log['pred'].to_numpy()),
...
forecast_results['lower'] = np.minimum(
...
errors = forecast_results['bottles_sold'] - forecast_results['forecast']
...
forecast_start_plot <- as.Date(reticulate::py_to_r(reticulate::py$forecast_start_plot))
