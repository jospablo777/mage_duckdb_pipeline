...
Running the pipeline in your terminal might be an option, but not the most appealing one, depending on your use case. For instance, the Iowa Liquor data set is updated once per month with the new data available on the 1st of each month, according to the [documentation.](https://data.iowa.gov/Sales-Distribution/Iowa-Liquor-Sales/m3tr-qhgy/about_data) Hence, it makes sense to run an update job once per month. We can configure a trigger for this, and it is very easy to do, so go review Mage's [documentation](https://docs.mage.ai/orchestration/triggers/configure-triggers-in-code) if you are in the need.

![](img/serving_done_data_lifecycle_logos.png)

And our data is ready to be served via SQL! We can continue with the pipeline's end goal: analytics and modeling.
...
The output table of this query is stored in `bottles_sold_per_type`. Let's have a peek at it:
...
Great! Now, we have a map of the stores that recently ordered mezcal. We can look for the closest one and go there for our treat. You can also click the dot of your store of interest to get its name and address.

![](img/analytics_done_data_lifecycle_logos.png)
...
We can write the query as a docstring and then pass it to the `.sql()` command. Also, notice that we get our query results as a Pandas data frame with the `.df()` at the end.
...
# Filter the data to include only rows for training and testing
pivoted_data['year_month'] = pd.to_datetime(pivoted_data['year_month'])
...
With our disjoint sets ready, we can continue encoding our categorical `month` variable using the one-hot encoding method.
...
X_train = X_train.drop(columns=['month']).join(X_train_encoded)
...
First, the forecaster is instantiated, and then we wrap it into the `ConformalIntervals` class to get a conformal predictor, this way we will be able also to predict a nice confidence interval, adding an extra layer of uncertainty quantification for our predictions. Then, we train it with the `fit` function. With our learned model at hand, we can now move to forecasting. So, let's predict the rum sales:
...
# Print results
print("Forecasted rum sales for the last two quarters of 2019:")
print(y_pred)

print("\n")
print("Forecasted rum sales confidence interval (90% coverage) for the last two quarters of 2019:")
print(pred_int)
```
...
# Plot prediction intervals as separate lines for debugging
...
That's great, but can we add another model for benchmarking? 🤩
...
![](img/ml_done_data_lifecycle_logos.png)
