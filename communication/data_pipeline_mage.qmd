...
We will begin by converting our retrieved information into a simple features object (`sf`). The `sf` library greatly simplifies geospatial analysis, especially when working with geospatial data types. First, we transform the `store_location` column (which is a string) into a `POINT` geometry type using the `st_as_sfc` function. Specifying the coordinate reference system (`crs = 4326`) is crucial here.
