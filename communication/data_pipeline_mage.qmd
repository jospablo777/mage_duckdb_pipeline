...
Let's say we want to know how well we sell liquor and see if there is a trend in our historical data. We start by aggregating our freshly queried data. One advantage of using R is that its libraries have pretty useful abstractions; for example, we will use `floor_date` from `lubricate` to aggregate the data by month.
...
Great! Now we have a map of the stores that recently made an order of mezcal.
