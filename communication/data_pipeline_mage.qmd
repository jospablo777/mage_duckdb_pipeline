...
Notice two things. First, we restrict Python to the 3.12 series because this project was tested in that runtime. Second, `mage-ai` is pinned to `0.9.75`, while DuckDB allows compatible newer versions starting from `1.5.2`. The exact versions installed in your environment are recorded in `uv.lock`, so collaborators and future runs can recreate the same working setup.
...
uv add "mage-ai==0.9.75" "duckdb>=1.5.2"
uv sync
```
