...
After we have our connection in Python, we can query the data we will need for the model.
...
```{python}
pivoted_data = data_df.pivot(index='year_month', columns='liquor_type', values='bottles_sold').reset_index()

# Fill missing values (just in case)
pivoted_data = pivoted_data.fillna(0)

# Add the `month` as a categorical feature
pivoted_data['month'] = pd.to_datetime(pivoted_data['year_month']).dt.month_name()
...
pivoted_data = pivoted_data.sort_values(by='year_month')  # Ensure chronological order
...
y = pivoted_data[target]  # Target variable
X = pivoted_data[features]  # Features

# Define test period (last two quarters of 2019)
train_cutoff = pivoted_data['year_month'] < '2019-07-01'
test_cutoff = ~train_cutoff

X_train, X_test = X[train_cutoff], X[test_cutoff]
...
# Replace the original 'month' column with encoded columns
...
# Assign forecast values for the test period
results_2019.loc[results_2019['year_month'] >= '2019-07-01', 'Forecast'] = y_pred.values
...
plt.axvline(x=pd.to_datetime('2019-07-01'), color='#ed7c65', linestyle='--', label='Start of test period')

# Add titles and labels
plt.title('Actual vs. forecasted rum sales in 2019')
plt.xlabel('Month')
...
That's nice, but can we add an uncertainty quantification layer with conformal prediction to get a nice prediction interval? 🤩
